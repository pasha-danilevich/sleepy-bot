...
async def get_selected_date(dialog_manager: DialogManager, **kwargs):
    return {"selected_date": dialog_manager.dialog_data['selected_date']}


async def get_dream_text(dialog_manager: DialogManager, **kwargs):
    return {"dream_text": 'И мне приснилось как я...'}


async def get_dream_rating(dialog_manager: DialogManager, **kwargs):
